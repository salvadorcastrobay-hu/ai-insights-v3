...
Usage:
    python -m src.cli setup                              # Create tables + seed taxonomy
    python -m src.cli ingest                             # Fetch Fathom + HubSpot + match deals
    python -m src.cli ingest --source fathom             # Only Fathom
    python -m src.cli ingest --source hubspot            # Only HubSpot
    python -m src.cli ingest --match-only                # Re-run matching only
    python -m src.cli ingest --since 2024-01-01          # Incremental from date
    python -m src.cli run                                # Full batch (all transcripts)
    python -m src.cli run --sample 5 --model gpt-4o      # Direct API, 5 transcripts
    python -m src.cli run --dry-run                      # Generate JSONL only
    python -m src.cli run --resume                       # Resume interrupted batch
    python -m src.cli status                             # Check batch status
    python -m src.cli qa --sample 30                     # QA evaluate 30 transcripts
    python -m src.cli qa --report                        # View last QA report
    python -m src.cli qa --apply                         # Apply QA refinements
    python -m src.cli embed                              # Embed transcripts for RAG search
    python -m src.cli embed --since 2024-06-01           # Incremental embedding
    python -m src.cli embed --force                      # Re-embed everything
    python -m src.cli backfill-summaries                 # Backfill Fathom summaries
"""
...
from src.connectors.supabase import get_client, execute_schema_direct
from src.connectors.seed_taxonomy import run_seed
from src.agents.orchestrator import run_pipeline, get_batch_status
from src.agents.ingestion_agent import run_ingestion
from src.connectors.fathom import fetch_summary
from src.agents.qa_agent import run_qa, print_report, apply_refinements
from src.agents.embedding_agent import run_embedding_pipeline
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m src.cli setup                              Setup DB and taxonomy
  python -m src.cli ingest                             Fetch Fathom + HubSpot + match
  python -m src.cli ingest --source fathom --since 2024-06-01
  python -m src.cli ingest --match-only                Re-run deal matching
  python -m src.cli run --sample 5 --model gpt-4o      Validate with 5 transcripts
  python -m src.cli run                                 Full batch (all transcripts)
  python -m src.cli run --resume                        Resume interrupted batch
  python -m src.cli qa --sample 30                      QA evaluate 30 transcripts
  python -m src.cli qa --report                         View last QA report
  python -m src.cli qa --apply                          Apply QA refinements
  python -m src.cli status                              Check batch progress
  python -m src.cli backfill-summaries                  Backfill Fathom summaries
        """,
    )
