...
import { getPaidRefreshJob } from "@/lib/competitor-ads/paid-refresh-job";
...
export async function GET(request: Request): Promise<Response> {
  ...
  const job = getPaidRefreshJob(jobId);
  if (!job) return Response.json({ error: "No hay refresh de ads pagos en curso." }, { status: 404 });
